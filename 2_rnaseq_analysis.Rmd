...
v1 <- plotDGEvolcano(dge_compartment_bcells_all,LIMITS=c(0, 30),title = "B cell DGE CSF vs. PB",
                     with_labels = TRUE,show_top_genes=20,cluster_filt = "CSF")
v2 <- plotDGEvolcano(dge_compartment_tcells_all,LIMITS=c(0, 300),title = "T cell DGE CSF vs. PB",
                     with_labels = TRUE,show_top_genes=20,cluster_filt = "CSF")
...
levels(bcell_obj_csfpb) <- c("Expanded Plasma cells","Non-expanded Plasma cells","Expanded Memory B-cells","Non-expanded Memory B-cells","Expanded Naive B-cells","Non-expanded Naive B-cells")
...
### a. B cells PC
```{r,fig.height=8, fig.width=11,echo=TRUE,warning=FALSE}
target_genes <- dge_bcell_csfpb[dge_bcell_csfpb$cluster %in% c("Expanded Plasma cells","Non-expanded Plasma cells"),]
...
barplot(bcell_pathways, showCategory = 9)
```

#### Network of genes shared between pathways
```{r,fig.height=7, fig.width=11,echo=TRUE,warning=FALSE}
bcell_pathways_net <- pairwise_termsim(bcell_pathways,showCategory = 9)
emapplot(bcell_pathways_net, showCategory = 9)
```
