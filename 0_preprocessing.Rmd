...
# Pre-generated SingleR results - provided for the workshop
results_singler_fh <- "objects/singleR_celltype_annotations.rds"
...
seurat.obj <- findDoublets(seurat.obj,sample.col = "sample",threads = 8)
...
singler_results_list <- readRDS(results_singler_fh)
...
idx <- match(rownames(seurat.obj@meta.data), rownames(seurat.temp@meta.data))
stopifnot(!anyNA(idx))

annot_meta <- seurat.temp@meta.data[idx, c("main_bencode","fine_bencode","main_monaco","fine_monaco"), drop = FALSE]
...
- If you don't already have a reference installed, you can install the human PBMC referece with the following command:
- **InstallData("pbmcref.SeuratData")**

```{r}
### Install reference if you don't have it. The line is current commented out
# SeuratData::InstallData("pbmcref.SeuratData")
