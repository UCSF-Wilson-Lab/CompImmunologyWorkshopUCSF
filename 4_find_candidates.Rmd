...
bcr_graph_all<- graph_from_data_frame(bcr_full_edges, directed=F,vertices=bcr_full_node_df)
```
...
tcr_graph_exp<- graph_from_data_frame(tcr_exp_edges, directed=F,vertices=tcr_exp_node_df)
```
...
# Mac OS
irtools.osx -c param_file_gliph2.cfg
```
...
VDJdb <https://vdjdb.cdr3.net/> 1. You will need to create an account or login temporarily. 2. Once logged in, click on the 'Annotation' tab 3. Click on the 'Upload' button 4. Click 'Add' and select the file we are generating bellow
...
- experimental controls for mAbs help to eliminate antigens (phage) that bind to the antibody backbone
