...
- overlap single cell RNA-Seq and VDJ data and proceed with analyzing overlapping cells
...
# 4. Show overlap for TCR and BCR

### a. make dataframe of all VDJ cells and its overlaps

```{r}
# Compartment
bcr_df$COMPARTMENT <- tstrsplit(bcr_df$unique_cell_id,"_")[[2]]
tcr_df$COMPARTMENT <- tstrsplit(tcr_df$unique_cell_id,"_")[[2]]

overlap_df_bcr <- bcr_df[bcr_df$locus %in% "IGH",c("unique_cell_id","COMPARTMENT")]
overlap_df_tcr <- tcr_df[tcr_df$locus %in% "TRB",c("unique_cell_id","COMPARTMENT")]

# Merge while keeping all rows and all columns
merged_df <- merge(
  overlap_df_bcr,
  overlap_df_tcr,
  by = "unique_cell_id",
  all = TRUE,
  suffixes = c("_bcr", "_tcr")
)

# Add status columns
merged_df$BCR_status <- ifelse(
  !is.na(merged_df$COMPARTMENT_bcr),
  "BCR",
  NA
)

merged_df$TCR_status <- ifelse(
  !is.na(merged_df$COMPARTMENT_tcr),
  "TCR",
  NA
)

# Optional: combined classification
merged_df$cell_status <- dplyr::case_when(
  !is.na(merged_df$COMPARTMENT_bcr) & !is.na(merged_df$COMPARTMENT_tcr) ~ "TCR_and_BCR",
  !is.na(merged_df$COMPARTMENT_bcr) ~ "BCR_only",
  !is.na(merged_df$COMPARTMENT_tcr) ~ "TCR_only",
  TRUE ~ "Neither"
)

# RNA-Seq overlap
merged_df$rnaseq_overlap <- "no"
merged_df[merged_df$unique_cell_id %in% colnames(rnaseq_obj), "rnaseq_overlap"] <- "yes"
merged_df$sample <- tstrsplit(merged_df$unique_cell_id,"-")[[2]]
```

### b. barplots of overlaps

```{r}
# Define muted publication-style colors
sample_colors <- c(
  "28_CSF"  = "#6BAED6",  # lighter muted blue
  "32_CSF"  = "#2171B5",  # darker muted blue
  "28_PBMC" = "#FC9272",  # lighter muted red
  "32_PBMC" = "#CB181D"   # darker muted red
)
...
merged_df <- merged_df %>%
  mutate(
    cell_class = factor(
      cell_status,
      levels = c("TCR_only", "BCR_only", "TCR_and_BCR")
    ),
    sample = factor(
      sample,
      levels = c("28_CSF", "32_CSF", "28_PBMC", "32_PBMC")
    )
  )

# Count data
plot_df_all <- merged_df %>%
  filter(!is.na(cell_class)) %>%
  count(cell_class, sample, name = "n")

plot_df_rnaseq <- merged_df %>%
  filter(rnaseq_overlap == "yes", !is.na(cell_class)) %>%
  count(cell_class, sample, name = "n")
...
# Plot: all cells
p_all <- ggplot(plot_df_all,
                aes(x = cell_class, y = n, fill = sample)) +
...
# Plot: RNA overlap only
p_rnaseq <- ggplot(plot_df_rnaseq,
                   aes(x = cell_class, y = n, fill = sample)) +
  geom_col(width = 0.75, color = "black", linewidth = 0.2) +
  scale_fill_manual(values = sample_colors) +
  labs(
    ...
    y = "Cell count",
    fill = "Sample",
    title = "RNA-seq Overlapping Cells"
  ) +
  pub_theme
...
# 5. Plot cell before and after VDJ filter

### a. SingleR annotations pre and post VDJ filter

- annotations done at the cell level and make use of the raw counts layer
...
- annotations are informed by mapping normalized (SCT) cells onto an atlas which is also SCT normalized
...
# 8. Plot cell type annotations after re-clustering
...
plot_grid(p2_post_filter_singler,p4_reclustered_singler,ncol = 2)
```
...
plot_grid(p2_post_filter_azimuth,p4_reclustered_azimuth,ncol = 2)
```
...
- We should not see an defined clusters for other cells types since these data should only contain B cells and T cells
...
# Defining Expanded populations

# 1. define expanded clonotypes
...
### a. subset B cell and T cell objects
...
```{r fig.height=6, fig.width=7, warning=FALSE}
p14 <- dittoBarPlot(tcell_obj, "expansion_category", group.by = "status.compartment", main = "T cells",color.panel = dittoColors()[1:3])
p15 <- dittoBarPlot(bcell_obj, "expansion_category", group.by = "status.compartment", main = "B cells",color.panel = dittoColors()[2:3])
...
# Optional - Use Azimuth to clean up annotations

# 1. Map misannotated T cells

### a. make ref and query objects

```{r}
MAIN_TCELL_ANNOT <- c("CD4 T","CD8 T","other T")

## Make Reference Object
cell_ref_tcells <- names(tcell_obj$predicted.celltype.l1[tcell_obj$predicted.celltype.l1 %in% MAIN_TCELL_ANNOT])

tcell_ref_obj <- tcell_obj[,colnames(tcell_obj) %in% cell_ref_tcells]

## Query of non T cells
t_query_obj <- tcell_obj[,! colnames(tcell_obj) %in% cell_ref_tcells]
```

### b. re-normalize reference and query

```{r}
# Keep the trusted labels in the reference
tcell_ref_obj$tcell_ref_label <- tcell_ref_obj$predicted.celltype.l1

# change default assay
DefaultAssay(tcell_ref_obj)  <- "RNA"
DefaultAssay(t_query_obj)    <- "RNA"

tcell_ref_obj  <- SCTransform(tcell_ref_obj, verbose = FALSE)
t_query_obj    <- SCTransform(t_query_obj, verbose = FALSE)

tcell_ref_obj <- RunPCA(tcell_ref_obj, verbose = FALSE)
tcell_ref_obj <- RunUMAP(tcell_ref_obj, dims = 1:30, reduction = "pca",return.model = TRUE,verbose = FALSE)
```
...
### d. map annotation onto query

```{r}
t_query_obj <- MapQuery(
  anchorset = anchors,
  query = t_query_obj,
  reference = tcell_ref_obj,
  refdata = list(mapped_tcell_label = "tcell_ref_label"),
  reference.reduction = "pca",
  reduction.model = "umap"
)
```

# 2. Extract updated annotations

- optionally after completing the annotations, you could refine CD4/CD8 based on whether there is CD8 expression in GEX and CSP

```{r}
new_query_annot_df <- t_query_obj@meta.data[,c("predicted.celltype.l1","predicted.mapped_tcell_label","predicted.mapped_tcell_label.score")]
```

# 3. Update annotations

- Even after going through these steps we can see

```{r}
new_col_annot <- tcell_obj$predicted.celltype.l1

uni_query_celltypes <- unique(new_query_annot_df$predicted.mapped_tcell_label)
for (t_annot in uni_query_celltypes) {
  cells_to_re_annot <- row.names(new_query_annot_df[new_query_annot_df$predicted.mapped_tcell_label %in% t_annot,])
  new_col_annot[names(new_col_annot) %in% cells_to_re_annot] <- t_annot
}

tcell_obj$azimuth_updated_celltype_l1 <- new_col_annot

annot_compare_df <- tcell_obj@meta.data[,c("predicted.celltype.l1", "azimuth_updated_celltype_l1","sub.celltype.annot")]
```
