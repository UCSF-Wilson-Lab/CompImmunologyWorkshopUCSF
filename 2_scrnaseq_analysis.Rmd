...
suppressPackageStartupMessages(library(dreamlet))
...
# 3. Differential Gene Expression (DGE) CSF vs PBMC

- In this section we are using the standard DGE tools in Seurat
- For more complex analyses, it is worth exploring the **dreamlet** R package which implements pseudobulk analyses
...
# 4. Pseudobulk analysis - Cells that overlap VDJ

### a. pseudobulk aggregation

```{r}
# Seurat -> SingleCellExperiment
sce <- as.SingleCellExperiment(rnaseq_obj)

sce$status <- factor(sce$status,
                     levels = c("Healthy", "Disease"))

sce$compartment <- factor(sce$compartment,
                          levels = c("PB", "CSF"))

# Pseudobulk by donor-sample within the B-cell cluster
pb <- aggregateToPseudoBulk(
  sce,
  assay = "counts",
  cluster_id = "main.celltype.annot",
  sample_id = "sample",
  verbose = FALSE
)
```
...
```{r}
res.proc <- processAssays(pb, ~status + compartment, min.count = 1)
res.dl <- dreamlet(res.proc, ~status + compartment)
...
```{r}
p1 <- plotVolcano(res.dl, coef = "compartmentCSF") +
  ggtitle("compartmentCSF\n(CSF vs PB)")

p2 <- plotVolcano(res.dl, coef = "statusDisease") +
  ggtitle("statusDisease\n(Disease vs Healthy)")

p1 + p2
```

# 5. T cell DGE - Expanded vs Non-Expanded Clones

### a. run DGE on T cells Expanded vs. Non-expanded (CSF)

```{r echo=TRUE, fig.height=7, fig.width=8, warning=FALSE}
TOP_N <- 12
...
# 6. B cell DGE - Expanded vs Non-Expanded Clonotypes
...
target_genes <- dge_bcell_csfpb[dge_bcell_csfpb$cluster %in% c("Expanded Plasmablast","Expanded B memory","Expanded B intermediate","Expanded B naive"),]
...
p1_bcell <- cnetplot(edox, foldChange=de_fc_list)
...
## Network categorySize can be scaled by 'pvalue' or 'geneNum' 
p1_tcell <- cnetplot(edox, foldChange=de_fc_list)
