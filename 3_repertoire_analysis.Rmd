...
- Overall this tools seems very good very quick and preliminary
  exploration of repertoire data. In particular, the way clones are
  defined benefits TCR repertoire analysis more than BCR.

# 1. Quantify Clones

- CSF BCR has a higher percentage of unique clonotypes relative to PBMC
  when using 'strict' criteria

- When the scale is set to FALSE, it is shown there are more total
  unique clonotypes in PBMC
...
- clonotypes are sorted by abundance and binned in decending order from
  most abundance clonotypes ([1:10]) to the least
- changing to 'aa' cloneCall criteria leads to more total B cell
  clonotypes in PBMC
...
# 4. Shared clonotype scatter plots - TCR

- Disease patient has an increased number of TCR clones that are PBMC or
  CSF exclusive
- Healthy patient has more clones that are a even mixture between PBMC
  and CSF
...
# 5. Shared clonotypes - proportions
...
# 6. Clonal diversity measures

<https://en.wikipedia.org/wiki/Diversity_index>
<https://evolution.unibas.ch/walser/bacteria_community_analysis/2015-02-10_MBM_tutorial_combined.pdf>

- Richness - index emphasizing the number of different types of
  clonotypes
- Shannon - index that is a form species richness where rare OTU's play
  more of an influence
- Inv. Simpson - index emphasizes species dominance where proportional
  abundance among clonotype groups are averaged

```{r}
# BCR
p_bcr <- clonalDiversity(
  bcell_screp,
  cloneCall = "strict",
  chain = "IGH",
  group.by = "sample",
...
# Panel
p_bcr + p_tcr + plot_layout(ncol = 2)
```
...
- An automated threshold (using the GMM method) of 0.15 was outputted
  for the full cohort and was also used for this subset of samples

```{r}
dist_nearest <- distToNearest(bcr_results_igh,vCallColumn = "v_call",jCallColumn = "j_call",cellIdColumn = "unique_cell_id")
...
### b. BCR isotype switching summary
...
bcr_results_igh$patient <- tstrsplit(bcr_results_igh$sample,"_")[[1]]

iso_df <- bcr_results_igh %>%
...
- Local species diversity (in this case, used for B & T cell
  populations)

- Interpretation of X-axis diversity scores 0 : species richness (R)
  (general diversity index) 0-\>1 : Shannon Diversity =\> exponent of
  Shannon-Weiner index 1-\>2 : Simpson Diversity =\> inverse of the
  Simpson index 2-\>4 : inverse reciprocal of the largest clone,
  diversity of most abundant clone
...
- known 5-mer motifs observed in literature are highlighted in color
  (<https://www.frontiersin.org/articles/10.3389/fimmu.2013.00358/full>)
  - WA/TW
  - WRC/GYW
  - SYC/GRS
- <https://shazam.readthedocs.io/en/stable/vignettes/Targeting-Vignette/>
...
- Need larger clonotypes across more samples to effectively use this
  tool
...
- Since our data doesn't have one large BCR clonotype, lets generate an
  example tree with public data
